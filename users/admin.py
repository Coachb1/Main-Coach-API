...
class ClientUserInfoAdmin(admin.ModelAdmin):
    list_per_page = 10
    list_display = ('id','client_name','domain_name','member_emails','restricted_ids','demo_ids','accessed_bot_ids','coach_skills','coach_expertise','departments','restricted_pages','restricted_features','allowed_ips','allow_audio_interactions','make_new_user_in_trail','ui_information','help_text','heading','sub_heading','tag_line')
    list_filter = ('client_name',)
    search_fields = ('client_name','domain_name')
    list_editable = ('domain_name','member_emails','restricted_ids','demo_ids','accessed_bot_ids','coach_skills','coach_expertise','departments','restricted_pages','restricted_features','allowed_ips','allow_audio_interactions','make_new_user_in_trail','ui_information','help_text','heading','sub_heading','tag_line')
    ordering = ('-id',)
...
def sync_profile_and_bot_data(sender, instance, **kwargs):
    ...
    if bot_ids:
        bot_ids = [b_id.strip() for b_id in bot_ids.split(',')]
