        ...
        try:
            ...
            try:
                ...
                if client_obj.webhook_url:
                    ...
                    invoke_webhook("simulation-attempted",{"username":participant_attributes.get("username"),"client_id": client_obj.client_name if client_obj else "", "participant_email": participant_email, "simulation_title": test.title, "report_link": report_url, "date": str(test_attempt_session.created.date())},client_obj.webhook_url) 
            except Exception as e:
        ...
        if signature_bot.bot_scenario_case == 'icons_by_ai':
            try:
                ...
                if client_obj.webhook_url:
                    ...
                    invoke_webhook("bot-interaction",{"username":participant_attributes.get("username"),"client_id": client_obj.client_name if client_obj else "", "participant_email": participant_email, "Conversation_transcript": conv, "date": str(test_attempt_session.created.date())},client_obj.webhook_url) 
            except Exception as e:
                logger.info(f"Failed to invoke webhook")
        return Response({"status": "sent"}, status=status.HTTP_200_OK)
